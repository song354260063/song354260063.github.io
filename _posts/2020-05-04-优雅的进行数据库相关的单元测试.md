...
![测试框架类图 _3_.jpg](https://i.loli.net/2020/05/04/jIm1qDUpzZKHGAP.png)
...
此外还有两个类 一个用于记录 后缀及需要处理为临时表的表名集合，另一个是 ThreadLocal 用于将上述记录Object 传递给 MyBatisPlugin。比较简单直接参考代码吧。
...
import com.sinosoft.dynamic.aop.DataSourceContextHolder;
import com.sinosoft.dynamic.aop.DataTestContextHolder;
import com.sinosoft.dynamic.dto.DataTestSqlObject;
...
以上短短数行代码即可解决单元测试与数据库相关的交互问题，简单又实用，强大又优雅，如果问题，欢迎评论交流~
